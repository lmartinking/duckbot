...
async def test_on_message_normal_message(bot_user, stats, cmd):
    ...
    delattr(message.channel, "parent")  # Ensure it's a normal channel. Mock has a 'parent' attribute by default.
    ...
    stats.process_message.assert_called_once_with(message.channel.guild, message.channel, message.author, message)
...
async def test_on_message_command_message(bot_user, stats, cmd):
    ...
    cmd.process_message.assert_called_once_with(message.channel.guild, message.channel, message.author, message, bot=bot)
